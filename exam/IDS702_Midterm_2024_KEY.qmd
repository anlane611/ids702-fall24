...
Incorrect - p-value is not the probability that the null is true (1 pt), and we do not accept the null hypothesis (2pt)
...
Incorrect - we do not assume the alternative (1 pt), and we would not reject the null here (0.42\>0.05) (2 pt)
...
Incorrect - CI does not capture a probability because the true difference is not random (3 pts)
...
Incorrect - CI does not capture a probability - the observed difference is always in the CI (3 pts)
...
$\hat{Y} = 71.7 - 0.03(infant deaths)+0.0002(measles)+0.09(HepB)-9.7(Status=Developing)$

-0.5 for each of the following errors:

-   includes the error term

-   does not include the hat on Y

-   includes hats on either the estimate values or the predictors

-   Status variable is not defined correctly

-1.5 if none of the predictor variables are defined

\vspace{1cm}

b\. Write an interpretation for the coefficient estimate of the Status variable.

Controlling for all other predictors, life expectancy is 9.7 years lower in developing countries, on average.

-1 for each error:

-   controlling for other predictors (or similar accurate language)

-   switched developed/developing

-   "on average" or "average life expectancy"
...
$Y=\beta_0+\beta_1x_1+\beta_2x_2+\beta_3x_3+\beta_4x_4+\beta_5x_1x_4+\epsilon$

$Y$ = life expectancy; $x_1$ = infant deaths; $x_2$ = measles; $x_3$=hepatitis B; $x_4$ = 1 for Status=developing, 0 otherwise

-1 if any "hats" are included

-0.5 if missing error term (does not need to state that error term is normally distributed)

-1 if missing interaction term

-1 if Status variable is incorrectly defined (check question a; avoid double jeopardy)

\vspace{1cm}

d\. Write the implied theoretical models for developed and developing countries. You do not need to define the terms again.

Developed: $Y=\beta_0+\beta_1x_1+\beta_2x_2+\beta_3x_3+\epsilon$

\vspace{1cm}

Developing: $Y=(\beta_0+\beta_4)+(\beta_1+\beta_5)x_1+\beta_2x_2+\beta_3x_3+\epsilon$

\vspace{1cm}

-1.5 if developed and developing are switched but otherwise correct

-1.5 if $x_4$ is included

-0.5 if missing error term (check part d; avoid double jeopardy)

-1 if "hats" are included (check part d; avoid double jeopardy)
